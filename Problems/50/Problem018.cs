...
namespace Problems._50 {
    public class Problem018 : IProblem {
        //TODO - move this to reading from text file
        ...
        public void ProblemSolver() {
            int? maxSum = null;
            var rowIdx = numbers.Length - 2;
            while (true) {
                for (var index = 0; index < numbers[rowIdx].Length; index++) {
                    numbers[rowIdx][index] += Math.Max(numbers[rowIdx + 1][index], numbers[rowIdx + 1][index + 1]);
                    if (rowIdx == 0) {
                        maxSum = numbers[rowIdx][index];
                    }
                }
                for (var index = 0; index < numbers[rowIdx + 1].Length; index++) numbers[rowIdx + 1][index] = 0;
                rowIdx = --rowIdx;
                if (maxSum != null) {
                    break;
                }
